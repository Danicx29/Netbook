...
{ 
    ...
    $this->Image('../../../../web/img/general/musicworld3.png',1.2,1.4,3);  
    // Titulo
    ...
    $this->Cell(5,0.9,'Music World S.A de C.V ',0,0,'C',false); 
    $this->Cell(5.5);    
    $this->Cell(0,0.9,'Fecha: '.$fecha,0,1,'R',false); 
    $this->Cell(7.1);    
    $this->Cell(5,0.9,'Colonia el milagro Local #4, San Marcos, San Salvador',0,0,'C',false);
    ...
    $this->Cell(0,0.9,'Usuario: '.$_SESSION['usuario_public'],0,0,'C',false);
    // Line break
    $this->Ln(1.5);
    }
}
...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
    $this->Cell(0,1,'Inventario  ',0,0,'C',false);  
    // Line break
    $this->Ln(1);
}
// Page footer
...
{ //posicion 
    ...
    // Arial
    $this->SetFont('Arial','B',14); 
    $this->Cell(0,0.9,'Gracias por preferir Music World !!!',0,2,'C',false);
    // Page number
 ...
 function tabla($header, $data)  
{
    ...
    foreach($data as $row)
    {       
       $this->Cell($w[0],2.5,$row[1],'LR',0,'C',$fill);
        $this->Cell($w[1],2.5,$row[3],'LR',0,'C',$fill);
        $this->Cell($w[2],2.5,'$'.$row[5],'LR',0,'C',$fill);
        $this->Cell($w[3],2.5,$row[7],'LR',0,'C',$fill); 
       $this->Cell($w[4],2.5, $this->Image('../../../../web/img/albumes/'.$row[4], $this->GetX(), $this->GetY(),2.5),'LR',0,'C',false);//se cargan los datos
...
$header = array( 'Nombre del Album',utf8_decode('Año'),'Precio','Existencias','Foto'); 
//le asignamos margenes a la pagina
$fpdf->setMargins(1.1,1.1,1.1);
$fpdf->setTitle('Inventario');
...
$fpdf->SetFont('Arial',"",18);   
    ...
    $fpdf->Cell(0,1,'No hay ningun album registrado en el inventario',0,1,'C',false);
}
