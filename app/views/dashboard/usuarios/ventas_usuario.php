...
require_once("../../models/dashboard/usuarios/usuarios.class.php"); 
...
{ //posicion 
    ...
    // Arial
    $this->SetFont('Arial','B',14); 
    $this->Cell(0,0.9,'Ventas Netbook',0,2,'C',false);
    // Page number
 ...
 function tabla($header, $data)  
{
    ...
    {       
       ...
       $this->Cell($w[1],2.5,$row[3],'LR',0,'C',$fill);
        $this->Cell($w[2],2.5,$row[3],'LR',0,'C',$fill);
        $this->Cell($w[3],2.5,$row[5],'LR',0,'C',$fill);
        $this->Cell($w[4],2.5,'$'.$row[7],'LR',0,'C',$fill); 
       $this->Cell($w[5],2.5, $this->Image('../../../../web/img/libros/'.$row[4], $this->GetX(), $this->GetY(),2.5),'LR',0,'C',false);//se cargan los datos
...
$header = array( 'codigo venta','Libro','Autor','Pago','Precio','portada'); 
...
$fpdf->AddPage();  
$fpdf->titulo();  
$mtsUsuarios =new mtsUsuario(); 
$data=$mtsUsuarios->GetRporteVentas($_GET['codigo_U']); 
if($data!=null){ 
$fpdf->Ln(1);
...
$fpdf->SetFont('Arial',"",18);   
    ...
    $fpdf->Cell(0,1,'No hay ninguna venta asociada',0,1,'C',false);
}
