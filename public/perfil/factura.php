...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
    $this->Cell(0,1,utf8_decode('Factura para libro:'.$valores[0]),0,0,'C',false);  
    // Line break
    $this->Ln(1);
}
// Page footer
 ...
 function tabla($header, $data)  
{
    ...
    {       
       ...
       $this->Cell($w[4],2.5, $this->Image('../../web/img/libros/'.$row[4], $this->GetX(), $this->GetY(),2.5),'LR',0,'C',false);//se cargan los datos
...
$header = array( 'Portada','N venta','Nombre libro','Numero tarjeta','Precio');
...
session_start();
$perfil =new mi_perfil(); 
$data=$perfil->GetRporteVenta($_GET['venta']);
$venta=$perfil->Getdatoslibro($_GET['venta']);
$fpdf->AddPage();  
$fpdf->titulo($venta);  
if($data!=null){ 
$fpdf->Ln(1);
...
$fpdf->SetFont('Arial',"",18);   
    ...
    $fpdf->Cell(0,1,utf8_decode('No hay ninguna reseña asociada'),0,1,'C',false);
}
