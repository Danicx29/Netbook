...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
    $this->Cell(0,1,'Compras del usuario '.$valores[0],0,0,'C',false);  
    // Line break
    $this->Ln(1);
}
// Page footer
 ...
 function tabla($header, $data)  
{
    ...
    {       
       ...
       $this->Cell($w[5],2.5, $this->Image('../../web/img/libros/'.$row[5], $this->GetX(), $this->GetY(),2.5),'LR',0,'C',false);//se cargan los datos
...
$header = array( 'codigo','Libro','Autor','Pago','Precio','portada'); 
...
$usuario2=$mtsUsuarios->GetdatosUsuario($_GET['codigo_U']);
$fpdf->AddPage();

$fpdf->titulo($usuario2);  
