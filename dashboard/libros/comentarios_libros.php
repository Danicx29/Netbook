...
{ 
    ...
    // celda(ancho en cm,alto en cm,texto para mostrar,borde,ajustar celda,alineacion de la celda,color de fondo)
    $this->Cell(0,1,utf8_decode('Reseñas del libro'.$valores[0]),0,0,'C',false);  
    // Line break
    $this->Ln(1);
}
// Page footer
...
$header = array( 'usuario','nombre','apellido','comentario','estado'); 
...
session_start();
$libros =new libros(); 
$data=$libros->GetReporteValoraciones($_GET['libro']);
$libro=$libros->GetReporteLibros($_GET['libro']);
$fpdf->AddPage();  
$fpdf->titulo($libro);  
if($data!=null){ 
$fpdf->Ln(1);
